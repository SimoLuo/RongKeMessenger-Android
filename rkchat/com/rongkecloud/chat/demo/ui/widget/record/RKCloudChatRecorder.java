...
public class RKCloudChatRecorder{
	...
	private static final int AUDIO_TYPE_3GPP = 2;	
	// 声音采样率
	private static final int SAMPLE_BITRATE_AMR = 8000;
	private static final int SAMPLE_BITRATE_3GPP = 8000;
	// 编码采样率
	private static final int ENCODE_BITRATE_AMR = 12000;	
	...
	/*
	 * 构造函数
	 * <p>为保证录制的音频文件在多平台间正常播放，请尽量采用默认提供的录音格式
	 * vivo 和 oppo 手机录音不支持3gp格式，如果是这两种手机，应使用AUDIO_TYPE_AMR格式，其余的可直接使用默认的3gp格式即可
	 */
	private RKCloudChatRecorder(int audioType){
		if(AUDIO_TYPE_AMR == audioType){
			mAudioOutputFormat = MediaRecorder.OutputFormat.RAW_AMR;
			mAudioEncoder = MediaRecorder.AudioEncoder.AMR_NB;
			mExtension = ".amr";	
			mSampleRecordBit = SAMPLE_BITRATE_AMR;
			mEncodeRecordBit = ENCODE_BITRATE_AMR;
			
		}else if(AUDIO_TYPE_3GPP == audioType){
