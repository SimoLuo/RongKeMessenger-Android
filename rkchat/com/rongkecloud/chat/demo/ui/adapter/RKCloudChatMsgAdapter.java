...
public class RKCloudChatMsgAdapter extends BaseAdapter{
	...
	public int getItemViewType(int position) {
		int type = -1;
		RKCloudChatBaseMessage msgObj = (RKCloudChatBaseMessage) mAllData.get(position);
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if(0 == position){
			...
			RKCloudChatBaseMessage beforeMsgObj = (RKCloudChatBaseMessage) mAllData.get(position-1);	
		...
		if(ITEM_STYLE_TIP == type){
			...
			if(MSG_DIRECTION.SEND == msgObj.getDirection()){
				...
				if(MSG_STATUS.RECEIVE_DOWNING==msgObj.getStatus() 
						&& (System.currentTimeMillis()-mMmsManager.getMmsDownTime(msgObj.getMsgSerialNum())>=PROCESS_SENDING_OR_DOWNING_MAXTIME)){
			...
			// 设置头像长点击事件
//			if(CHAT_TYPE.GROUP == mChatType){
//				itemBuffer.senderPhoto.setOnLongClickListener(new OnLongClickListener() {
//					@Override
//					public boolean onLongClick(View v) {
//						RKCloudChatMsgActivity activity = (RKCloudChatMsgActivity)mContext;
//						activity.appendContent(null!=contactObj ? contactObj.getShowName() : msgObj.getSender());
//						return true;
//					}
//				});
//			}else{
//				itemBuffer.senderPhoto.setOnLongClickListener(null);
//			}
			...
			if(MSG_DIRECTION.SEND == msgObj.getDirection()){
				...
				default:
					if(SingleChat.class.equals(mChatClassObj)){
						...
						if(!RKCloudChatConstants.FLAG_ADD_FRIEND_SUCCESS.equals(msgObj.getExtension())
								&& type!=ITEM_STYLE_SEND_AVCALL){
//							itemBuffer.sendStatus.setVisibility(View.VISIBLE);
				...
				if((ITEM_STYLE_RECEIVED_AVCALL==type ||  ITEM_STYLE_RECEIVED_MUTLIMEETING==type) 
						&& MSG_STATUS.RECEIVE_RECEIVED==msgObj.getStatus()){
			...
			if(ITEM_STYLE_SEND_AVCALL==type || ITEM_STYLE_RECEIVED_AVCALL==type){
				...
				if(msgObj instanceof TextMessage){
					...
					showVideoMsg(convertView, (VideoMessage)msgObj);
		...
		if(RKCloudChatConstants.FLAG_AVCALL_IS_AUDIO.equals(msgObj.getExtension()) 
			...
			if(msgObj instanceof ImageMessage){
				...
				if(MSG_STATUS.RECEIVE_RECEIVED==msgObj.getStatus() || MSG_STATUS.RECEIVE_DOWNFAILED==msgObj.getStatus()){
					...
					if(msgObj instanceof AudioMessage){ // 播放或关闭播放语音消息
						// 文件不存在时给出提示
					...
					}else if(msgObj instanceof VideoMessage){ // 表示播放视频消息
						// 文件不存在时给出提示
					...
					}else if(msgObj instanceof FileMessage){ // 打开文件消息
						// 文件不存在时给出提示
	...
	private void showAudioMsg(View convertView, final AudioMessage msgObj){
		...
		if(msgObj.getMsgSerialNum().equals(mAudioHelper.getPlayingMsgSerialNum())){
			// 正在播放中
			if(null == mCurrPlayAudioMsg){
				mCurrPlayAudioMsg = (AudioMessage)msgObj;
	 ...
	 * 视频消息的显示
	 */
	private void showVideoMsg(View convertView, VideoMessage msgObj){
		ImageView pic = (ImageView) convertView.findViewById(R.id.video_img);
		ImageView playImg = (ImageView) convertView.findViewById(R.id.video_icon);// 显示图片时显示该图标，图片未显示时该图标隐藏不显示
