...
public class RKCloudChatRecordVideoActivity extends RKCloudChatBaseActivity implements Callback{
	...
	private static final int UPDATE_RECORDING_INTERVALTIME = 100;// // 更新录制时长的间隔时间，单位：毫秒
	// UI元素
	private Button mSendedBtn;
	...
	private void initUIAndListeners(){
		...
		titleTV.setText(R.string.rkcloud_chat_videorecording_title);
		...
		mSendedBtn = (Button)findViewById(R.id.title_right_btn);
		mSendedBtn.setVisibility(View.VISIBLE);
		mSendedBtn.setText(R.string.rkcloud_chat_videorecording_btnconfirm);				
		mVideoView = (VideoView)findViewById(R.id.videoview);
		mStartLayout = (LinearLayout)findViewById(R.id.layout_start);
		mStartBtn = (ImageView)findViewById(R.id.start);
		mStopLayout = (LinearLayout)findViewById(R.id.layout_stop);
		...
		// 设置按钮的显示与隐藏状态
		mSendedBtn.setEnabled(false);
		mStartLayout.setVisibility(View.VISIBLE);
		mStopLayout.setVisibility(View.GONE);
		mSwitchCameraLayout.setVisibility(mEnableSwitchCamera ? View.VISIBLE : View.GONE);		
		// 点击事件
		mSendedBtn.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				...
				AlertDialog.Builder dialog = new AlertDialog.Builder(RKCloudChatRecordVideoActivity.this)
						...
						.setMessage(R.string.rkcloud_chat_videorecording_send_confirm)
						...
						.setPositiveButton(R.string.rkcloud_chat_btn_confirm, new DialogInterface.OnClickListener() {
							...
							public void onClick(DialogInterface dialog, int which) {
								mMsc = new MediaScannerConnection(RKCloudChatRecordVideoActivity.this, new MediaScannerConnectionClient() {
									@Override
	...
	private synchronized void setCameraId(){
		...
		try{
			...
			AlertDialog.Builder dialog = new AlertDialog.Builder(this)
					.setTitle(R.string.rkcloud_chat_tip)
					.setMessage(R.string.rkcloud_chat_videorecording_openfailed)
					.setCancelable(false)
					.setPositiveButton(R.string.rkcloud_chat_btn_confirm, new DialogInterface.OnClickListener() {
						@Override
						public void onClick(DialogInterface dialog, int which) {
	...
	private void startRecord(){
		...
		mMediarecorder.setVideoEncodingBitRate(384 * 1024);// 设置视频的比特率
		if (mFrameRate != -1) {
	...
	private void stopRecord(){
		...
		mSendedBtn.setEnabled(!TextUtils.isEmpty(mFilePath) && new File(mFilePath).exists());
		mStartLayout.setVisibility(View.VISIBLE);
		mStopLayout.setVisibility(View.GONE);
		mSwitchCameraLayout.setVisibility(mEnableSwitchCamera ? View.VISIBLE : View.GONE);
